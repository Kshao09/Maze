...
public class Maze {
    ...
    private void solveHelper(Cell source, Cell target) {
        ...
        if(source.deadend) {
            source.shortest = 100000;
            return;
        }

        source.visited = true;

        boolean canJump = source.horizontalJump + source.verticalJump > 0 && source.verticalJump + source.row <rows && source.horizontalJump + source.col < columns;

        if (source.row <rows - 1 && !map[source.row + 1] [source.col].visited && !source.noDown)// down cell is not discovered yet!
            solveHelper (map [source.row +1] [source.col], target);// call it recursively for bottom neighbor

        if (source.col < columns - 1 && !map[source.row][source.col + 1].visited && !source.noRight) // right cell is not discovered yet!
            solveHelper (map [source.row][source.col + 1], target); // call it recursively for right neighbor

        if (canJump && !map[source.verticalJump + source.row][source.horizontalJump + source.col].visited ) {
            solveHelper (map [source.verticalJump + source.row] [source.horizontalJump + source.col], target);
        }

        if (source.row== rows - 1 && !source.noRight) {
            source.shortest = map[source.row][source.col + 1].shortest;

            if (canJump) {
                source.shortest = Math.min(source.shortest, map[source.verticalJump + source.row][source.horizontalJump + source.col].shortest);
            }
            source.shortest++;

            if(canJump && source.shortest == map[source.verticalJump + source.row][source. horizontalJump + source.col].shortest +1) {
        ...
        } else if (source.col == columns - 1 && !source.noDown) {
            source.shortest = map[source.row + 1] [source.col].shortest;

            if(canJump) {
                source.shortest = Math.min(source.shortest-1, map [source.verticalJump + source.row][source.horizontalJump + source.col].shortest);
            }
            source.shortest++;
        ...
        } else if ((source.noDown || source.noRight) && (source.row == rows-1 || source.col == columns-1)){
            source.shortest = 100000;

        } else {
            Cell rightNeighbor = map[source.row][source.col + 1];
            Cell bottomNeighbor = map[source.row + 1] [source.col];

            int rightPath = source.noRight || rightNeighbor.deadend? 100000: rightNeighbor.shortest;
            int downPath = source.noDown || bottomNeighbor.deadend ? 100000: bottomNeighbor.shortest;

            source.shortest = Math.min (rightPath, downPath);
            ...
            }else if(source.shortest == rightNeighbor.shortest + 1) {
                source.nextStep=rightNeighbor;

            }else if(canJump && source.shortest == map[source.verticalJump + source.row][source.horizontalJump + source.col].shortest +1) {
                source.nextStep = map[source.verticalJump + source.row] [source.horizontalJump + source.col];

            }

        }
        source.shortest += source.wait;
